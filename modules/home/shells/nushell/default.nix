    ...
    shellAliases = lib.mkForce (
      lib.mapAttrs (
        _name: value: let
          ...
          transformedValue =
            if isComplexScript || hasPositionalParams || hasMultipleCommands
            then let
              lines = lib.splitString "\n" value;
              nonCommentLines =
                builtins.filter (
                  line: let trimmed = lib.trim line; in trimmed != "" && !lib.hasPrefix "#" trimmed
                )
                lines;
            ...
            in
              ...
              else "bash -c '" + (lib.replaceStrings ["'"] ["'\"'\"'"] cleanScript) + "'"
            else let
              words = lib.splitString " " value;
              conflictingCommands = ["find" "grep" "sort" "uniq" "head" "tail" "cut" "wc"];
              transformedWords = builtins.map (word:
                if builtins.elem word conflictingCommands
                then "command " + word
                else word)
              words;
              ...
              withEnvVars = lib.concatStrings (
                lib.imap0 (
                  i: part:
                    if i == 0
                    then part
                    else if lib.hasPrefix "(" part
                    then "$" + part
                    else if builtins.match "^[A-Za-z_][A-Za-z0-9_]*.*" part != null
                    then "$env." + part
                    else "$" + part
                )
                parts
